...
int mockCheckRetryTimerReturnValue = 1;

int checkRetryTimer()
{
	return mockCheckRetryTimerReturnValue;
}
...
// Test the parseMultipartDocument function with a valid boundary
...
// Test the parseMultipartDocument function with a NULL boundary
void test_parseMultipartDocument_NullBoundary() {
    WEBCFG_STATUS result;
    const char config_data[] = "HTTP 200 OK\nContent-Type: multipart/mixed; boundary=NULL\nEtag: 345431215\n";
    size_t data_size = strlen(config_data);
    char ct[] = "Content-Type: multipart/mixed; boundary=NULL";
    char *trans_uuid = strdup("1234");
    char *config_data_copy = (char *)malloc(data_size + 1);
    ...
    if (config_data_copy != NULL) {
        ...
        result = parseMultipartDocument(config_data_copy, ct, data_size, trans_uuid);
        CU_ASSERT_EQUAL(result, WEBCFG_FAILURE);
        free(config_data_copy);
    } else {
        CU_FAIL("Memory allocation for config_data_copy (NULL boundary) failed");
    }
}
...
void add_suites( CU_pSuite *suite )
{
    *suite = CU_add_suite( "tests", NULL, NULL );
	  ...
	  //CU_add_test( *suite, "test  parseMultipartDocument_NullBoundary", test_parseMultipartDocument_NullBoundary);  
